"""
Parameter Golf competitive submission: train_gpt_thorne.py
...
Techniques implemented:
  ...
  6. FP16 Embedding Passthrough
  7. Stochastic Weight Averaging (SWA)
  8. Exponential Moving Average (EMA)
  9. Multi-Token Prediction (MTP)
  10. Bigram Hash Embedding
  11. Parallel Muon + Parameter Banking
  12. Legal Score-First TTT
"""
...
class Hyperparameters:
    ...
    # [T9] Multi-Token Prediction
    mtp_num_heads = int(os.environ.get("MTP_NUM_HEADS", 2))
    mtp_loss_weight = float(os.environ.get("MTP_LOSS_WEIGHT", 0.2))

    # [T10] Bigram Hash Embedding
    bigram_vocab_size = int(os.environ.get("BIGRAM_VOCAB_SIZE", 2048))
    bigram_dim = int(os.environ.get("BIGRAM_DIM", 128))

    # [T7] Stochastic Weight Averaging
...
def eval_val_sliding(
    ...
    """Sliding window evaluation: each scored token sees maximum context."""
    seq_len = eval_seq_len or args.train_seq_len
...
# [T2] Int6 Quantization (per-row, stored in int8 containers)
...
def quantize_float_tensor(t: Tensor) -> tuple[Tensor, Tensor]:
    """Standard int8 quantization for non-int6 tensors."""
    t32 = t.float()
    if t32.ndim == 2:
        row_max = t32.abs().amax(dim=1)
        scale = (row_max / 127.0).clamp_min(1.0 / 127.0).to(INT8_PER_ROW_SCALE_DTYPE)
    ...
    q = torch.clamp(torch.round(t32 / scale.float()), -127, 127).to(torch.int8).contiguous()
    return q, scale
...
# [T2+T6] Mixed int6 quantization with fp16 embedding passthrough
def mixed_quantize_int6(state_dict: dict[str, Tensor], int6_cats: set[str]):
    ...
    for name, tensor in state_dict.items():
        ...
        if not t.is_floating_point() or t.numel() <= INT8_KEEP_FLOAT_MAX_NUMEL:
            ...
            continue
        # Control tensors: keep as fp32
        if any(p in name for p in CONTROL_TENSOR_NAME_PATTERNS):
            result[name] = t.float()
            meta[name] = "passthrough_ctrl"
            continue
        # [T6] Embeddings: keep as fp16 (too sensitive for quantization)
        if cat == "embed":
            result[name] = t.to(torch.float16)
            meta[name] = "passthrough_fp16"
            continue
        # [T2] Int6 for MLP and attention matrices
...
# [T3] CastedLinear with QAT/STE support (used for bigram proj, VE proj, mtp heads, lm_head)
class CastedLinear(nn.Linear):
    ...
    def forward(self, x: Tensor) -> Tensor:
        ...
        if CastedLinear._qat_enabled and self.training and w.ndim == 2:
            ...
            # STE: forward uses quantized values, backward passes through original
            w = w + (w_q - w).detach()
        bias = self.bias.to(x.dtype) if self.bias is not None else None
        return F.linear(x, w, bias)


def _maybe_qat(w: Tensor, dtype: torch.dtype) -> Tensor:
    """Apply int6 STE quantization to a 2D bank weight slice when QAT is active."""
    w_cast = w.to(dtype)
    if CastedLinear._qat_enabled and w.requires_grad and w.ndim == 2:
        with torch.no_grad():
            w32 = w.float()
            row_max = w32.abs().amax(dim=1)
            scale = (row_max / 31.0).clamp_min(1.0 / 31.0)
            w_q = (torch.clamp(torch.round(w32 / scale[:, None]), -32, 31) * scale[:, None]).to(dtype)
...
# [T5+T14] NTK-Aware RoPE Scaling with Partial RoPE
class Rotary(nn.Module):
    ...
    def forward(self, seq_len: int, device: torch.device, dtype: torch.dtype) -> tuple[Tensor, Tensor]:
        if (
            ...
            # [1, T, 1, D/2] for (B,T,H,D) layout used by FA3
...
# [T14] Partial RoPE: only rotate first rope_dims dimensions
...
# [T13] CausalSelfAttention with FA3 + [T12] XSA + [T14] Partial RoPE
# No CastedLinear weights -- Q/K/V/Out come from bank parameters
class CausalSelfAttention(nn.Module):
    ...
    # [T12] XSA: subtract self-value projection (GQA-aware)
    ...
    def forward(self, x: Tensor, q_w: Tensor, k_w: Tensor, v_w: Tensor, out_w: Tensor,
        ...
        # Apply QAT STE to bank weight slices
        q_w_c = _maybe_qat(q_w, x.dtype)
        k_w_c = _maybe_qat(k_w, x.dtype)
        v_w_c = _maybe_qat(v_w, x.dtype)
        out_w_c = _maybe_qat(out_w, x.dtype)
        # (B, T, H, D) layout -- native for FA3, transposed for SDPA fallback
        q = F.linear(x, q_w_c).reshape(bsz, seqlen, self.num_heads, self.head_dim)
        k = F.linear(x, k_w_c).reshape(bsz, seqlen, self.num_kv_heads, self.head_dim)
        v = F.linear(x, v_w_c)
...
class MLP(nn.Module):
    ...
    def forward(self, x: Tensor, up_w: Tensor, down_w: Tensor) -> Tensor:
        up_w_c = _maybe_qat(up_w, x.dtype)
        down_w_c = _maybe_qat(down_w, x.dtype)
        x = torch.relu(F.linear(x, up_w_c))
        return F.linear(x.square(), down_w_c)


class SmearGate(nn.Module):
    """Learned gate mixing current and previous-position embeddings."""
...
class BigramHashEmbedding(nn.Module):
    ...
    def bigram_hash(self, tokens: Tensor) -> Tensor:
        ...
        out[..., 0] = mod  # first position has no predecessor
...
# [T15] Value Embedding -- reinject token identity into attention values
...
class Block(nn.Module):
    ...
    def forward(self, x: Tensor, x0: Tensor, q_w: Tensor, k_w: Tensor, v_w: Tensor,
        ...
        attn_out = self.attn(self.attn_norm(x), q_w, k_w, v_w, out_w, v_embed=v_embed)
        x = x + self.attn_scale.to(dtype=x.dtype)[None, None, :] * attn_out
        x = x + self.mlp_scale.to(dtype=x.dtype)[None, None, :] * self.mlp(self.mlp_norm(x), up_w, down_w)
        return x
...
class GPT(nn.Module):
    def __init__(
        ...
        self.blocks = nn.ModuleList([
            Block(model_dim, num_heads, num_kv_heads, mlp_mult, rope_base, qk_gain_init, train_seq_len, rope_dims)
            for _ in range(num_layers)
        ])

        # [T12] XSA on last N layers
        if xsa_last_n > 0:
            for i in range(max(0, num_layers - xsa_last_n), num_layers):
                self.blocks[i].attn.use_xsa = True

        # [T15] Value Embedding -- shared table with per-layer scales
        ...
        # [T9] Multi-Token Prediction heads (training-only)
    ...
    def forward(self, input_ids: Tensor, target_ids: Tensor) -> Tensor:
        ...
        x = self._run_blocks(x0, x0, input_ids)
...
def main() -> None:
    ...
    log0(f"model_params:{n_params} (mtp_params:{mtp_params})")
    log0(f"world_size:{world_size} grad_accum_steps:{grad_accum_steps}")
    log0(f"compressor:{_COMPRESSOR}")
    xsa_layers = [i for i, b in enumerate(base_model.blocks) if b.attn.use_xsa]
    log0(f"flash_attn_3:{_USE_FA3} xsa_last_{args.xsa_last_n}:{xsa_layers} rope_dims:{args.rope_dims}")
    ...
    while True:
        ...
        # [T3] Late QAT: enable when LR drops below threshold
    ...
    # [T8] Apply EMA weights (preferred over raw final weights)
    if ema_state is not None:
    ...
    # [T9] Exclude MTP heads from export (training-only)
    ...
    # [T2+T4+T6] Int6 quantization + zstd compression + fp16 embedding passthrough
    ...
    if _COMPRESSOR == "zstd":
        ...
        raw = dctx.decompress(quant_blob_disk)
    else:
    ...
    # Build fresh eval model (no MTP heads)
    eval_model = GPT(
    ...
    # [T1] Sliding window evaluation on quantized model
    ...
    if args.eval_stride > 0 and args.eval_stride < sw_seq_len:
        ...
        # This is the final submission score
