...
class Muon(torch.optim.Optimizer):
    def __init__(
        ...
        backend_steps: int,
        nesterov: bool = True,
        momentum_warmup_start: float | None = None,
        ...
        super().__init__(
            ...
            dict(
                ...
                momentum_warmup_start=(
                    momentum if momentum_warmup_start is None else momentum_warmup_start
                ),
    ...
    def step(self, closure=None):
        ...
        for group in self.param_groups:
            ...
            if not params:
                continue
            lr = group["lr"]
            momentum_target = group["momentum"]
            momentum_start = group["momentum_warmup_start"]
            warmup_steps = group["momentum_warmup_steps"]
            frac = (
                min(self._step_count / warmup_steps, 1.0) if warmup_steps > 0 else 1.0
            )
            momentum = (1 - frac) * momentum_start + frac * momentum_target
...
class Rotary(nn.Module):
    ...
    def __init__(self, args):
        super().__init__()
        dim = args.model_dim // args.num_heads
        base = args.rope_base
...
class CausalSelfAttention(nn.Module):
    def __init__(self, args):
        ...
        self.q_gain = nn.Parameter(
            torch.full((num_heads,), args.qk_gain_init, dtype=torch.float32)
        )
...
class Block(nn.Module):
    def __init__(self, args):
        super().__init__()
        dim = args.model_dim
        self.attn_norm = RMSNorm(args)
        self.mlp_norm = RMSNorm(args)
...
class GPT(nn.Module):
    def __init__(self, args):
        ...
        if args.logit_softcap <= 0.0:
            raise ValueError(
                f"logit_softcap must be positive, got {args.logit_softcap}"
            )
        ...
        self.blocks = nn.ModuleList([Block(args) for i in range(args.num_layers)])
...
def get_distributed_config() -> tuple[bool, int, int, int, int]:
    ...
    return distributed, rank, world_size, local_rank, grad_accum_steps


def get_cuda_device(local_rank: int) -> torch.device:
...
def clone_model_state(model: nn.Module) -> dict[str, Tensor]:
    return {name: t.detach().cpu().clone() for name, t in model.state_dict().items()}


def clone_optimizer_state(
    optimizers: list[torch.optim.Optimizer],
) -> list[dict]:
    return [copy.deepcopy(opt.state_dict()) for opt in optimizers]


def load_tokenizer(
    tokenizer_path: str, expected_vocab_size: int
) -> spm.SentencePieceProcessor:
    if not tokenizer_path.endswith(".model"):
        raise ValueError(
            f"Script only setup for SentencePiece .model file: {tokenizer_path}"
        )
    sp = spm.SentencePieceProcessor(model_file=tokenizer_path)
    if int(sp.vocab_size()) != expected_vocab_size:
        raise ValueError(
            f"VOCAB_SIZE={expected_vocab_size} does not match tokenizer vocab_size={int(sp.vocab_size())}"
        )
    return sp
...
def main() -> None:
    ...
    def step_optimizers() -> None:
        for opt in optimizers:
            opt.step()

    def run_microstep_batch() -> Tensor:
    ...
    if args.warmup_steps > 0:
        initial_model_state = clone_model_state(base_model)
        initial_optimizer_states = clone_optimizer_state(optimizers)
        model.train()
    ...
    def maybe_set_stop_after_step(step: int, approx_training_time_ms: float) -> None:
        """Latch ``stop_after_step`` when the wallclock cap is reached.

        All-reduces the cap check so every rank agrees on the stopping step;
        once latched, the outer loop validates and exits at that step.
        """
        nonlocal stop_after_step
        reached_cap = (
    ...
    while True:
        ...
        maybe_set_stop_after_step(step, approx_training_time_ms)
    ...
    q_val_loss, q_val_bpb = run_eval_val()
    torch.cuda.synchronize()
    log0(
